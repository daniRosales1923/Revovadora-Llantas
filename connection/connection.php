<?php
	...
	class ConexionDatos{
		...
		function EjecutaConsulta($Consulta){
			global $Conexion;
			$Resultado = pg_query($Conexion,$Consulta) or die("Error al Ejecutar Consulta: $Consulta" . pg_last_error($Conexion)); 
			return $Resultado;
		}
		function DatosUsuario($Usr, $Pwd){
            ...
            if ($Usr!=""){
                ...
                if ($Pwd!=""){
                    ...
                    return $this->EjecutaConsulta("Select U.idusuario, U.idusuario, U.nombre, U.apellidopaterno, U.apellidomaterno, U.correo, U.telefono,  U.usuario, U.status from usuario AS U ". $Condicion);
                }
                else{
